...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func spawnPipes() {
        ...
        let pipeScale: CGFloat = 0.5
        let centerY = self.frame.size.height / 2 - 30
        let randomOffset = CGFloat(arc4random_uniform(200))
        let pipeGap = 315.0

        let pipeDown = SKSpriteNode(texture: pipeDownTexture)
        pipeDown.position = CGPoint(x: 0.0, y: centerY - pipeGap + randomOffset - 0)
        pipeDown.zPosition = -1
        pipeDown.setScale(pipeScale)
        pipeDown.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: pipeDown.size.width * pipeScale * 0.6, height: pipeDown.size.height * (pipeScale + 0.3)))
        ...
        pipeUp.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: pipeUp.size.width * pipeScale * 0.6, height: pipeUp.size.height * (pipeScale + 0.3)))
